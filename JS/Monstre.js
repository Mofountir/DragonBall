...
export class Monstre extends objetGraphique {
  ...
  draw(ctx) {
    ...
    //Effet de collision
    if (this.effetCollition.actif) {
      ctx.rotate(this.effetCollition.rotation);
      ctx.scale(this.effetCollition.echelle, this.effetCollition.echelle);
    }
    ...
    ctx.beginPath();
    ctx.moveTo(0, -15);
    ctx.quadraticCurveTo(decalageAntenne, -25, decalageAntenne, -35);
    ctx.stroke();
